...
const HoldSlides_1 = ({ title }) => {
  ...
  return (
    <div className="HoldSlides">
      <h3 className="section-title-findhold" aria-label={title}>{title}</h3>
      <Carousel
        ...
        {Trending.map((hold, index) => (
          <div key={index} className="carousel-thumbnail">
            <Link to={`/findhold/${hold.id}`} aria-label={hold.title}>
              ...
              <h5 className="image-title" aria-label={hold.title}>{hold.title}</h5>
            </Link>
          </div>
        ))}
      </Carousel>
